...
from .forms import TodoCreateForm, TodoCreateForm, TodoUpdateForm
...
def create(request):
    if request.method == 'POST':
        ...
        if form.is_valid():# you dont need else because django takes care of it itself -> error handling
...
def update(request, todo_id): # django documant, model forms
    todo = Todo.objects.get(id=todo_id)
    if request.method == 'POST':
